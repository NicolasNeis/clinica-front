<template>
  <v-card width="100%" height="100%" color="white" class="mx-auto" style="border-radius: 10px">
    ...
    <div class="mx-16">
      <v-text-field v-model="doctor.name" class="mt-10 mb-2" label="Nome Completo" variant="outlined" density="compact"
        :rules="[rules.required]"></v-text-field>
      <v-text-field v-model="doctor.id" label="ID" class="mb-n3" variant="outlined" v-if="routeId" :disabled="true"
        density="compact"></v-text-field>
      <v-text-field v-model="doctor.crm" class=" mb-2" label="CRM" variant="outlined" density="compact"
        :rules="[rules.required]" @input="limitInputCRM()"></v-text-field>
      <v-text-field v-model="doctor.specialization" class=" mb-n3" label="Especialidade" variant="outlined"
